...
export function ExamSetupPage({ config }: ExamSetupPageProps) {
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 p-5 lg:flex-row lg:items-start lg:gap-8 lg:p-9 lg:px-10">
      <div className="flex min-w-0 flex-1 flex-col gap-5">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-10">
          <h1 className="font-serif text-[28px] font-medium tracking-[-0.8px] text-ink lg:text-4xl">{texts.title}</h1>
          <div className="flex flex-col items-start gap-3 sm:items-end">
            <LanguageSwitch value={testLanguage} label={texts.examLanguage} onChange={setTestLanguage} />
            <SessionNavLink showSignIn={false} />
          </div>
        </header>

        <section className="flex flex-col gap-3">
          <h2 className="sr-only">{texts.requiredSectionLabel}</h2>
        ...
        <section className="flex flex-col gap-3">
          <h2 className="sr-only">{texts.profileSectionLabel}</h2>
